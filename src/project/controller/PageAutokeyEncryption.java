...
public class PageAutokeyEncryption {
    ...
    public void Encrypt(){
        ...
        vBox.getChildren().add(new Label(""));
        vBox.getChildren().add(new Label(" Step 2: Now, for each letter in the message, we replace the letter with, the letter found in the tabula recta, on row, corresponding to the letter from the message and column, corresponding to the letter from the key."));
        vBox.getChildren().add(new Label(""));
        ...
        for (int i = 0; i < message.length() ; i++) {
            ...
            vBox.getChildren().add(new Label(" For letter  " + message.charAt(i) + "  from row " + row + " (corresponding to letter " + message.charAt(i) + " from the message) and column " + column + "(corresponding to letter " + key.charAt(i) + " from the key),  we get letter:   " + tabulaRecta[row][column]));
    ...
    public void showTabulaRecta(){
        vBox.getChildren().add(new Label(""));
        vBox.getChildren().add(new Label(" This is the Tabula recta, the table used for Autokey encryption: "));
        vBox.getChildren().add(new Label(""));
