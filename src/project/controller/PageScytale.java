    ...
    public void Encrypt(){
        try{
            ...
            if (!(message.matches("[a-zA-Z ]+$"))){
                ...
                if (!(keyTextField.getText().matches("[0-9]+")) || Integer.parseInt(keyTextField.getText()) <= 0 || Integer.parseInt(keyTextField.getText()) >= message.length()){
                    keyLabel.setText("Please insert a natural non-null number, smaller than the number of letters in the message!");
                }else {
                    key = Integer.parseInt(keyTextField.getText());

                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/project/views/pageScytaleEncryption.fxml"));
                    Parent root = loader.load();

                    PageScytaleEncryption pageScytaleEncryption = loader.getController();
                    pageScytaleEncryption.transferMessage(message, key);


                    Stage primaryStage = new Stage();
                    Scene scene = new Scene(root, 775, 430);
                    primaryStage.setTitle("Scytale Cipher Encryption");
        ...
        try {
            ...
            if (!(message.matches("[a-zA-Z ]+$"))){
                ...
                if (!(keyTextField.getText().matches("[0-9]+")) || Integer.parseInt(keyTextField.getText()) <= 0 || Integer.parseInt(keyTextField.getText()) >= message.length()){
                    ...
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/project/views/pageScytaleDecryption.fxml"));
                    Parent root = loader.load();

                    PageScytaleDecryption pageScytaleDecryption = loader.getController();
                    pageScytaleDecryption.transferMessage(message, key);


                    Stage primaryStage = new Stage();
                    Scene scene = new Scene(root, 775, 430);
                    primaryStage.setTitle("Scytale Cipher Decryption");
                    primaryStage.setScene(scene);
                    primaryStage.show();
                }
            }
