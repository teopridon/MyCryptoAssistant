...
public class PageCaesarDecryption {
        ...
        for (int i = 0; i < message.length() ; i++) {
            for (int j = 0; j < 26; j++) {
                if (message.charAt(i)==englishAlphabet[j]){
                    ...
                    if ((j - key)%26 < 0){
                        String s = " " + englishAlphabet[j] + "  from position  " + j + ",    turns to:   " + englishAlphabet[26 + ((j - key)%26)] + "   which is situated on position  " + (26 + ((j - key)%26)) + "  in the english alphabet";
