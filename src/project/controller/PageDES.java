...
public class PageDES {
    ...
    public void Encrypt(){
        try{
            ...
            if (!(message.matches("[0-9a-fA-F]+")) || !(message.length() == 16)){
                messageLabel.setText("Please insert a message that contains only hexadecimal digits and that is exactly 16 characters long.");
            }else {
                message = transformToUppercase(message);

                if (!(key.matches("[0-9a-fA-F]+")) || !(key.length() == 16)){
                    keyLabel.setText("Please insert a key that contains only hexadecimal digits and that is exactly 16 characters long.");
                }else {
                    key = transformToUppercase(key);

                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/project/views/pageDesEncryption.fxml"));
                    Parent root = loader.load();

                    PageDESEncryption pageDesEncryption = loader.getController();
                    pageDesEncryption.transferMessage(message, key);


                    Stage primaryStage = new Stage();
                    Scene scene = new Scene(root, 775, 430);
                    primaryStage.setTitle("DES Encryption");
        ...
        try{
            ...
            if (!(message.matches("[0-9a-fA-F]+")) || !(message.length() == 16)){
                ...
                if (!(key.matches("[0-9a-fA-F]+")) || !(key.length() == 16)){
                    ...
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("/project/views/pageDesDecryption.fxml"));
                    Parent root = loader.load();

                    PageDESDecryption pageDesDecryption = loader.getController();
                    pageDesDecryption.transferMessage(message, key);


                    Stage primaryStage = new Stage();
                    Scene scene = new Scene(root, 775, 430);
                    primaryStage.setTitle("DES Decryption");
                    primaryStage.setScene(scene);
                    primaryStage.show();
                }
            }
