...
public class PageMonoalphabeticSubstitutionDecryption {
    ...
    public void Decrypt(){
        ...
        for (int i = 0; i < message.length() ; i++) {
            ...
            for (int j = 0; j < 26 ; j++) {
                if (message.charAt(i) == alphabet[j][1]){

                    vBox.getChildren().add(new Label(" Letter  " + message.charAt(i) + "  is replaced with letter:  " + alphabet[j][0]));
                    result = result + alphabet[j][0];
                    break;
                }
            }
        }
